namespace Questar.OneRoster.ApiFramework
{
    ...
    public abstract class OneRosterController : Controller
    {
        ...
        protected async Task<IActionResult> GetCollectionAsync<T>(IQueryable<T> queryable, CollectionEndpointRequest<T> request)
            where T : class
        {
            ...
            var count = queryable.CountAsync(context);

            HttpContext.Response.Headers.Add("X-Total-Count", count.ToString());

            var linkHeaderValues = BuildLinkHeaderValues(request, await count);
            ...
            {
                { "StatusInfoSet", response.StatusInfoSet },
