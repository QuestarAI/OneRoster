...
namespace Questar.OneRoster.Test.Collections
{
    public class ScopedDictionary<TKey, TValue> : IDictionary<TKey, TValue>
    {
        ...
        {
            return _items.Remove(key) || _parent.Remove(key);
        }

        public bool TryGetValue(TKey key, out TValue value)
        {
            return _items.TryGetValue(key, out value) || _parent?.TryGetValue(key, out value) == true;
        }
        ...
        {
            return _items.ContainsKey(key) || _parent?.ContainsKey(key) == true;
        }
        ...
        {
            return _items.Contains(item) || _parent.Contains(item);
        }
        ...
        {
            return _items.Remove(item) || ((ICollection<KeyValuePair<TKey, TValue>>) _parent).Remove(item);
        }
    }
}